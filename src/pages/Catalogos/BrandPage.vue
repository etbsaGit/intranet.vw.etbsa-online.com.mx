<template>
  ...
  >
    <q-card style="width: 100%; height: 100%">
      ...
      <q-item>
        <q-item-section>
          <brand-form ref="edit" :brand="selectedItem" />
        </q-item-section>
      </q-item>
    </q-card>
  </q-dialog>
...
import BrandForm from "src/components/Catalogos/BrandForm.vue";
...
const getRows = async () => {
  let res = await sendRequest("GET", null, "/api/intranet/brand", "");
  rows.value = res;
};
...
const postItem = async () => {
  ...
  let res = await sendRequest("POST", final, "/api/intranet/brand", "");
  showAdd.value = false;
  getRows();
};
...
const destroyItem = async () => {
  let res = await sendRequest(
    "DELETE",
    null,
    "/api/intranet/brand/" + selectedItem.value.id,
    ""
  );
